...
int checkConditions(int n, int * colup, int * coldown, int * rowleft, int * rowright, int map[n][n], int i, int j) {
    ...
    for (k; k < n; k++) {
        if (map[i][k] == map[i][j] && k != j) {
            printf("No se cumple la condición de que no se repite en la fila\n");
            return 0;
        }
        if (map[k][j] == map[i][j] && k != i) {
            printf("No se cumple la condición de que no se repite en la columna\n");
            return 0;
        }
    }

    printf("checkConditions(%d, %d, %d)\n", i, j, map[i][j]);
    // comprobamos que se cumple la condición de la vista desde arriba
    k = 0;
    int count = 0;
    int max = -1;
    for (k; k < n; k++) {
        printf("RECORRO de arriba a abajo: %d. Cont ahora es %d\n", map[k][j], count);
        if (map[k][j] >= max || map[k][j] == 0) {
            max = map[k][j];
            count++;
        }
    }
    if (count > colup[j] && colup[j] < max) {
        printf("No se cumple la condición de la vista desde arriba porque %d != %d\n", count, colup[j]);
        return 0;
    }
    printf("Se cumple la condición de la vista desde arriba\n");
    // comprobamos que se cumple la condición de la vista desde abajo
    k = n-1;
    count = 0;
    max = -1;
    for (k; k >= 0; k--) {
        printf("RECORRO de abajo a arriba: %d Cont ahora es %d\n", map[k][j], count);
        if (map[k][j] >= max || map[k][j] == 0) {
            max = map[k][j];
            count++;
        }
    ...
    if (count > coldown[j] && coldown[j] < max) {
        printf("No se cumple la condición de la vista desde abajo porque %d != %d\n", count, coldown[j]);
        return 0;
    }
    printf("Se cumple la condición de la vista desde abajo\n");
    // comprobamos que se cumple la condición de la vista desde la izquierda
    k = 0;
    count = 0;
    max = -1;
    for (k; k < n; k++) {
        if (map[i][k] >= max || map[i][k] == 0) {
            max = map[i][k];
            count++;
        }
    }
    if (count > rowleft[i] && rowleft[i] < max) {
        printf("No se cumple la condición de la vista desde la izquierda porque %d != %d\n", count, rowleft[i]);
        return 0;
    }
    printf("Se cumple la condición de la vista desde la izquierda\n");
    ...
    if (count > rowright[i] && rowright[i] < max) {
        printf("No se cumple la condición de la vista desde la derecha porque %d != %d\n", count, rowright[i]);
        return 0;
    }
    printf("Se cumple la condición de la vista desde la derecha\n");
...
int tryToSolve(int n, int * colup, int * coldown, int * rowleft, int * rowright) {
    ...
    if (backtrack(n, colup, coldown, rowleft, rowright, map, 0, 0) == 1) {
        printf("Hemos encontrado una solución!\n");
        printGamePro(n, colup, coldown, rowleft, rowright, map);
        return 1;
    }
    printf("No hemos encontrado ninguna solución\n");
    return 0;
}
